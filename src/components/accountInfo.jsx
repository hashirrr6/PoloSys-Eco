...
import {
  ...
  RefreshCcw, Lock, Shield, Bell, Tag, HelpCircle, Mail, LogOut, Moon, Sun, ShieldCheck
} from 'lucide-react';
    ...
    >
      <div className="rounded-2xl p-6">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="mb-8">
          ...
          <div className={`flex items-center justify-between p-3 ${darkMode ? 'bg-gray-700 hover:bg-gray-600' : 'bg-white hover:bg-gray-50'} rounded-xl transition cursor-pointer`} onClick={toggleDarkMode}>
            <div className="flex items-center gap-3">
              {darkMode ? <Sun size={18} className="text-indigo-500" /> : <Moon size={18} className="text-red-500" />}
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
          <div className="flex items-center gap-3 p-3 text-red-500 rounded-xl transition cursor-pointer hover:bg-red-100 dark:hover:bg-red-800">
            <LogOut size={18} />
      ...
      <div className="space-y-2">
        {items.map((item, idx) => (
          <motion.div
            ...
            whileHover={{ x: 5 }}
            whileTap={{ scale: 0.98 }}
            className={`flex items-center justify-between p-3 ${darkMode ? 'bg-gray-700 hover:bg-gray-600' : 'bg-white hover:bg-gray-50'} rounded-xl transition cursor-pointer`}
          >
            <div className="flex items-center gap-3">
              {item.icon}
...
function NotificationToggle({ label, enabled, onToggle, icon, darkMode }) {
  return (
    <motion.div whileHover={{ x: 5 }} whileTap={{ scale: 0.98 }} className={`flex items-center justify-between p-3 ${darkMode ? 'bg-gray-700 hover:bg-gray-600' : 'bg-white hover:bg-gray-50'} rounded-xl transition cursor-pointer`} onClick={onToggle}>
      <div className="flex items-center gap-3">
        {icon}
...
function Toggle({ enabled, onChange }) {
  return (
    <motion.button
      ...
      onClick={onChange}
      whileTap={{ scale: 0.9 }}
      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-300 ease-in-out focus:outline-none ${enabled ? 'bg-red-400' : 'bg-gray-300'}`}
    >
      <motion.span
        layout
